...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir='', names=(), on_plot=None):
        ...
        fig, ax = plt.subplots(1, 1, figsize=(12, 9),tight_layout=True) #tight_layout=True
        nc, nn = self.nc, len(names)  # number of classes, names
        sn.set(font_scale=3 if nc < 50 else 0.8)  # for label size
        ...
        ax.set_xlabel('True',fontname="Arial",size=28)
        ax.set_ylabel('Predicted',fontname="Arial",size=28)
        ax.set_title(title,fontname="Arial",size=35,fontweight='bold')
        ax.tick_params(axis='both', labelsize=15)  #类别的大小
...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(), on_plot=None):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ax.plot(px, y, linewidth=2, label=f'{names[i]}')  # plot(recall, precision)
    else:
        ax.plot(px, py, linewidth=1, color='grey')  # plot(recall, precision)

    # ax.plot(px, py.mean(1), linewidth=3, color='blue', label='all classes %.3f mAP@0.5' % ap[:, 0].mean())
    ax.plot(px, py.mean(1), linewidth=4, color='blue', label='all classes')
    ax.set_xlabel('Recall',fontname="Arial",size=30)
    ax.set_ylabel('Precision',fontname="Arial",size=30)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.tick_params(axis='both', labelsize=20)
    # ax.legend(bbox_to_anchor=(1.04, 1), loc='upper left')
    ax.legend(fontsize=20)
    ax.set_title('Precision-Recall Curve', fontname="Arial",size=35, fontweight='bold')
...
def plot_mc_curve(px, py, save_dir=Path('mc_curve.png'), names=(), xlabel='Confidence', ylabel='Metric', on_plot=None):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py):
            ax.plot(px, y, linewidth=2, label=f'{names[i]}')  # plot(confidence, metric)
    else:
        ax.plot(px, py.T, linewidth=3, color='grey')  # plot(confidence, metric)

    y = smooth(py.mean(0), 0.05)
    # ax.plot(px, y, linewidth=4, color='blue', label=f'all classes {y.max():.2f} at {px[y.argmax()]:.3f}')
    ax.plot(px, y, linewidth=4, color='blue', label=f'all classes')
    ax.set_xlabel(xlabel,fontname="Arial",size=30)
    ax.set_ylabel(ylabel,fontname="Arial",size=30)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.tick_params(axis='both', labelsize=20)
    # ax.legend(bbox_to_anchor=(1.04, 1), loc='upper left')
    ax.legend(fontsize=20)
    ax.set_title(f'{ylabel}-Confidence Curve',fontname="Arial",size=35,fontweight='bold')
    fig.savefig(save_dir, dpi=250)
    plt.close(fig)
    if on_plot:
        on_plot(save_dir)
